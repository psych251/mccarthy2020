...
# Pilot A Notice
This is a pilot run on non-naive participants (n=2) to test the experiment and analysis pipeline.
Pilot A experiment can be accessed here (for a limited time):
[pilotA live experiment link](https://cogtoolslab.org:8860/experiment.html)

The below analyses and methods have outputs on Pilot A sample, not the actual replication sample.
...
The resulting output CSV was stored as `data/pilotA/preprocessed/block_silhouette_mccarthy2020cogsci_replication_pilotA.csv`.
...
df.full = read.csv(paste('../data/pilotA/preprocessed/block_silhouette_',iterationName,'_good.csv', sep=''))
...
F1 score on completed block towers, limear mixed effects model.
```{r}
